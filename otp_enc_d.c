...
int main(int argc, char *argv[])
{
	...
	// If socket could not be created, print an error
	if (listenSocketFD < 0) {
		fprintf(stderr, "otp_enc_d error: unable to open listening socket\n");
		exit(1);
	}
	...
	if (bind(listenSocketFD, (struct sockaddr *)&serverAddress, sizeof(serverAddress)) < 0) {
		fprintf(stderr, "otp_enc_d error: unable to bind socket\n");
		exit(1);
	}
	...
	while(1) {
		...
		if (establishedConnectionFD < 0) {
			fprintf(stderr, "otp_enc_d error: unable to accept connection\n");
			exit(1);
		}
		...
		if (pid == -1) {
			fprintf(stderr, "otp_end_d error: fork failed\n");
			exit(1);
		}
		...
		// child process
			...
			if (charsRead < 0) { 
				fprintf(stderr, "otp_enc_d error: issue receiving encode/decode flag from client\n");
				exit(1);
			}
			
			/**************************************************************************************************/
						
			// Check to see if client is not trying to encode
			if (strcmp(buffer, "encode") != 0) {
				...
				do {
					...
					// Print an error if we are unable to send the message back to the client
					if (charsWritten < 0) {
						fprintf(stderr, "otp_enc_d error: issue sending correct server message to client\n");
						exit(1);
					}
			...
			char plaintext[70000], key[70000], readBuffer[700];
			...
			while (strstr(plaintext, "@@") == NULL) 
			{
				...
				// Check for errors
				if (charsRead < 0) { 
					fprintf(stderr, "otp_enc_d error: issue receiving plaintext from client\n");
					exit(1);
				}
			...
			do {
				...
				if (charsWritten < 0) {
					fprintf(stderr, "otp_enc_d error: issue sending plaintext-receive success to client\n");
					exit(1);
				}
			} while (charsWritten < 7);
			...
			while (strstr(key, "@@") == NULL) 
			{
				...
				// Check for errors
				if (charsRead < 0) { 
					fprintf(stderr, "otp_enc_d error: issue receiving key from client\n");
					exit(1);
				}
			...
			// Buffer to hold the final cipher message
			char cipher[70000];
			memset(cipher, '\0', sizeof(cipher));
			
			// Convert plaintext characters into ascii values and store into table
			int i = 0;
			...
			for (i = 0; i < strlen(key); i++) {
				...
				// Add the ascii value the existing ascii value in the array
				...
				// Build cipher from chars
				cipher[i] = c;
			}
			
			// Prepare the cipher by adding sentinel to it
			...
			do {
				...
				// Return the cipher back to the client
				charsWritten = send(establishedConnectionFD, cipher, strlen(cipher), 0); 
				
				// Return an error if we are unable to send back cipher
				if (charsWritten < 0) {
					fprintf(stderr, "otp_enc_d error: issue sending cipher to client\n");
					exit(1);
				}
